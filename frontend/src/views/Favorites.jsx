...
export default function Favorites({
    ...
    const favoritedTools = allTools.filter(t => favorites.includes(t.id));avoritedTools = allTools.filter(t => favorites.includes(t.id));
    const favoriteWorkflows = allWorkflows.filter(w => savedWorkflows.includes(w.id));

    const renderEmptyState = (iconName, title, desc, btbText, btnColorClass) => {
        const Icon[iconName] || Icons.HelpCircle;
        return (
            <div className="glass-card rounded-3xl p-16 text-center max-w-xl mx-auto">
                <div className={`h-12 w-12 rounded-2xl flex items-center justify-center mx-auto mb-6 ${btnColorClass}/10 border border-${btnColorClass}/20 text-${btnColorClass}-400`}>
            ...
            className={`inline-flex items-center gap-1.5 px-4 rounded-xl text-xs front-semibold text-white transiton cursor-pointer ${
                btnColorClass === "indigo" ? "bg-indigo-600 hover:bg-indigo-500" : "bg-pink-600 hover:bg-pink-500"
            }`}
        >
            <span>{btnText}</span>
        </buttton>
...
return (
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-10 relative z-10">
        <div className="flex items-center gap-2 mb-8">
            ...
            >
                <Icons.ArrowLeft classNane="w-3.5 h-3.5" />
        ...
        favoritedTools.length === 0 ? (
            ...
            favoriteWorkflows.length === 0 ? (
                renderEmptyState("Layers", "No saved workflows yet", "Bookmark customized pipelines to view them here.", "Expore Pipelines, "pink")
            ) : (
